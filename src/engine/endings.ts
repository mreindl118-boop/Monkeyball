...
import type { Character, EndingType, GameState, Relationship, SetRelationship } from '../types'
...
export const ENDINGS: Readonly<Record<EndingType, EndingInfo>> = {
  ...
  open: {
    ...
    direction: (n) =>
      `${n.name} and ${player(n)} are together on an open agreement they both actually like; other partners are part of the picture, not a problem. Let that ease show: warm, unjealous, a little proud of how well it works.`,
  },
  polycule: {
  ...
  sacrifice: {
    ...
    direction: (n) =>
      `${n.name} chooses ${n.rival?.trim() || 'their work, or a life that was already waiting for them'} over ${player(n)}. ${player(n) === 'the player' ? 'The player' : player(n)} was great; something mattered more. Play it as a kind, sad goodbye with no villain.`,
  },
  reconciliation: {
...
export function selectEnding(ctx: EndingContext): EndingChoice {
  ...
  if (c?.relationshipStyle === 'monogamous' && (agreement === 'none' || agreement === 'casual')) {
  ...
  if (agreement === 'open' || agreement === 'poly') {
    return { type: 'open', reason: `You and ${first} are on ${agreement === 'open' ? 'an open' : 'a poly'} agreement you both like.` }
  }
  return { type: 'good', reason: `High trust and affection with ${first}, and honest the whole way.` }
}

/** The epilogue's story direction for a chosen ending. */
export function endingDirection(
  choice: Pick<EndingChoice, 'type' | 'group'>,
  ctx: { characterId: string; name: string; player?: string; names?: Record<string, string>; rival?: string },
): string {
